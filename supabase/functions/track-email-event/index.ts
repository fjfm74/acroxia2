...
// 1x1 transparent GIF
const PIXEL_GIF = Uint8Array.from(atob("R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7"), (c) =>
  c.charCodeAt(0),
);
...
function isFailureEvent(eventType: string): boolean {
  return ["bounced", "bounce", "complained", "complaint", "failed", "rejected", "blocked", "delivery_failed"].some(
    (x) => eventType.includes(x),
  );
}
...
function shouldAutoUnsubscribe(eventType: string, failureDetail: string): boolean {
  ...
  if (
    !(event.includes("bounce") || event.includes("failed") || event.includes("rejected") || event.includes("blocked"))
  ) {
    return false;
  }
